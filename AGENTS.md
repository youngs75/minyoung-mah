...
사용자와의 모든 소통은 항상 한국어로 진행합니다. 사용자 facing 메시지(에러, 진행, 문서)는 한국어입니다.

**코드 주석/docstring은 영문과 한글 병기**입니다. minyoung-mah는 PyPI에 공개된 라이브러리이고 docstring은 IDE hover/`help()`/PyPI 페이지에 노출됩니다. 동시에 현재 모든 컨슈머와 본인이 한국어 사용자이므로, 영문만 두면 사고 손실이 발생하고 한글만 두면 외부 공개 여지가 좁아집니다. 이를 모두 고려해 다음 형식을 따릅니다:

- **모듈/함수 docstring 첫 줄**: 영문 요약 한 줄. 그 다음 줄(또는 빈 줄 후 단락)에 한글 설명 추가.
- **inline 주석(`#`)**: 짧으면 `# English — 한글`, 길면 두 줄로 분리하여 영문 줄 + 한글 줄.
- **신규 코드는 처음부터 병기**, 기존 코드는 만질 때 병기로 갱신.
...
- **본 프로젝트의 글로벌 메모리 파일:** `~/.claude/global-memory/minyoung-mah.md`
- **세션 시작 시:** 위 파일을 읽고(필요 시 `cd ~/.claude/global-memory && git pull --rebase -q` 선행) 장기 궤적과 규칙을 적용
- **장기 기억 승격:** 프로젝트 로컬 auto memory(`~/.claude/projects/.../memory/`)는 scratch로만 사용. 다른 머신에서도 필요한 것, 프로젝트 철학·방향·경계, 여러 세션에 걸쳐 재사용된 교훈은 즉시 `minyoung-mah.md`로 승격
