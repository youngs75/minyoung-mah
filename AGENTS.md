...
├── .gitignore
├── minyoung_mah/                   # library 패키지 (active)
│   ├── core/                       # 6 protocols, Orchestrator, ExecuteToolsStep, registries
│   ├── hitl/                       # Null/Terminal/Queue HITL channels
│   ├── memory/                     # SqliteMemoryStore + NullMemoryStore (store.py만 active)
│   ├── model/                      # Single/Tiered ModelRouter
│   ├── observer/                   # Null/Collecting/Structlog/Composite observer
│   ├── resilience/                 # ResiliencePolicy + ProgressGuard (policy.py/progress_guard.py만 active)
│   ├── subagents/                  # (Phase 2b에서 정리 예정 — 현재 broken coding agent 사본)
│   ├── tools/                      # (Phase 2b에서 정리 예정 — 동일)
│   ├── cli/                        # (Phase 2b에서 정리 예정 — 동일)
│   ├── utils/
│   ├── config.py                   # (Phase 2b에서 재작성 예정)
│   ├── models.py
│   └── logging_config.py
├── tests/
│   └── library/                    # library 단위 테스트 (33개, active)
├── docs/
│   ├── design/                     # library-facing 설계 문서만 유지
...
- **Phase 1 — Bootstrap & Design Sketch** ✅ 완료
  - 폴더 구조 + git 초기화 + 원본 코드 import
  - Design sketch 작성 (library-facing 01/04만 유지, 소비자 매핑 02/03은 archive)
- **Phase 2a — Library 뼈대 구축** ✅ 완료
  - 6 protocol 정의 + opinionated default 구현 + 28 tests
  - Co-design 과정에서 `ExecuteToolsStep` 추가 + 5 tests
  - 현재 `tests/library/` 33개 전부 통과
- **Phase 2b — Library 안정화** 🔄 진행 예정
  - `minyoung_mah/{subagents,tools,cli,config.py}` 등 broken 사본 모듈 정리 (삭제 또는 library 품질로 재작성)
  - `datetime.utcnow()` deprecation 제거
  - `run_loop` 설계 확정 (currently `NotImplementedError`)
  - Langfuse observer 실제 구현
  - resilience watchdog/safe_stop을 library 품질로 재작성 (현재는 `coding_agent.*` import에 의존해 broken)
  - `minyoung_mah/memory/{extractor,middleware,schema}.py`, `minyoung_mah/resilience/{error_handler,retry_policy,watchdog,safe_stop}.py` 중 남길 것 / 버릴 것 결정
  - library 경계 재정의로 Phase 3(apt-legal 구축)·Phase 4(coding agent 이식)은 **이 리포의 로드맵에서 제외**. 해당 작업은 각 소비자 리포에서 진행.
...
- `minyoung_mah/` 하위 중 `subagents/`, `tools/`, `cli/`, `config.py`, `memory/{extractor,middleware,schema}.py`, `resilience/{error_handler,retry_policy,watchdog,safe_stop}.py` 등은 원본 coding agent 사본 상태로 남아 있고 broken이다 (import하지 않는다). Phase 2b 정리 대상.
- 경계 원칙: 이 리포 안에서 "특정 vertical agent를 위한 코드를 추가하는" 유혹이 들면 멈추고, library로 추상화 가능한지 먼저 점검한다. 추상화가 어색하면 그건 소비자 리포에 있어야 할 코드다.
