...
| 3 | `Orchestrator` | 역할을 조립·실행 (`run_pipeline`, `invoke_role`) | `minyoung_mah.core.orchestrator.Orchestrator` |
...
- ❌ Dynamic driver-role loop — `run_loop`은 0.1.0에서 전면 삭제됨. 소비자는 `invoke_role` 위에서 자기 loop를 직접 조립합니다 (코딩 에이전트 같은 동적 역할이 필요할 때).
...
- Phase 2b — 제자리 클린업 ✅ (2026-04-15) — broken 사본 모듈 전부 제거
- 0.1.0 — 소비자 피드백 반영 ✅ (2026-04-15) — apt-legal 실소비자 gap 3건: `StaticPipeline.shared_state`, `PipelineStepResult.payload_as`, `RoleInvocationResult.format_for_llm`(INCOMPLETE 배너). 죽은 `run_loop` shape 삭제, `langchain-core` required, `default_resilience` fallback 180s. 43 tests.
