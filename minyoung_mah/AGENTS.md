...
- 모든 모듈은 **런타임 의존성 `pydantic` + `structlog` 두 개**만 가정합니다. langchain 메시지 타입은 `_invoke_structured` / `_invoke_loop`처럼 **함수 안에서 lazy import**해서 optional extra로 유지합니다.
...
2. 새 파일이 하드 의존성을 추가하는가? → `pyproject.toml`의 runtime deps와 충돌하지 않는지 확인. 선택 의존은 함수 내부 import.
3. Observer 이벤트 이름을 추가/변경했는가? → `observer/events.py::EVENT_NAMES`에 등록하고, `docs/ARCHITECTURE.md`의 이벤트 표도 갱신.
4. `tests/library/`에서 33개 기존 테스트가 통과하는가? 새 동작이면 테스트도 함께 추가.
5. 소비자(`../apt-legal-agent/`, `../ax_advanced_coding_ai_agent/`)의 코드를 **이 리포 안에서** 수정하지 않았는가? — atomic 2-리포 커밋은 금지.
