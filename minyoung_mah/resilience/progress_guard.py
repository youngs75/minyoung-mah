"""Progress Guard — agent loop progress monitor.
Progress Guard — 에이전트 루프 진전 감시.

Detects repeated identical tool calls (stall) to prevent infinite loops.
...
라이브러리 차원 의미: guard 는 슬라이딩 윈도우 안에서 반복되는
``(tool_name, args_hash)`` 쌍을 추적한다. 도메인별 사이클 탐지(예: coding
agent 의 ``TASK-NN`` 위임 루프)를 위해 애플리케이션은 호출별 안정 secondary
key 를 반환하는 ``key_extractor`` callable 을 주입할 수 있다. extractor 가
``None`` 을 반환하면 해당 호출의 secondary 추적은 건너뛴다.
"""
...
class ProgressGuard:
    """Inspects an agent's tool-call pattern to decide whether it has stalled.
    에이전트의 도구 호출 패턴을 분석하여 정체 여부를 판단한다.

    Parameters
    ----------
    window_size:
        Recent action history window size. / 최근 행동 기록 윈도우 크기.
    stall_threshold:
        Stall verdict if any single action in the window appears at least this often.
        윈도우 내 동일 행동 빈도가 이 값 이상이면 정체로 판정.
    max_iterations:
        Absolute iteration cap. / 절대 반복 상한.
    """
    ...
    def disabled(cls) -> "ProgressGuard":
        ...
        절대 발화하지 않는 guard — iteration 횟수가 구성 단계에서 이미 bound
        된 파이프라인용 (예: 정적 파이프라인).
        """
        return cls(
            window_size=1,
    ...
    def record_action(self, tool_name: str, tool_args: dict) -> None:
        ...
        ----------
        tool_name:
            Name of the invoked tool. / 호출된 도구 이름.
        tool_args:
            Argument dict passed to the tool. / 도구에 전달된 인자 딕셔너리.
        """
        ...
        #
        # 주입된 extractor 를 통한 secondary-key 추적. extractor 가 무엇을
        # "같은 대상"으로 칠지 결정한다(예: TASK-NN id, 파일 경로, DB 테이블
        # 이름). None 을 반환하면 추적을 건너뛴다.
    ...
    def check(self, iteration: int) -> GuardVerdict:
        """Render a verdict for the current state.
        현재 상태를 판정한다.

        Parameters
        ----------
        iteration:
            Current loop iteration counter. / 현재 루프 반복 횟수.

        Returns
        -------
        GuardVerdict
            One of OK / WARN / STOP. / OK / WARN / STOP 판정.
        """
        # 1. Absolute iteration cap exceeded — 절대 반복 상한 초과
        ...
        # 2. Repeated secondary-key (domain-specific cycle break)
        ...
        # 3. Stall detection (frequency of same action within the window)
        ...
        # Reset the warn flag once the stall clears.
        # 정체가 해소되면 경고 플래그 리셋
        self._warn_issued = False
        return GuardVerdict.OK

    def get_stall_summary(self) -> dict:
        """Return a summary of the current recording state.
        현재 기록 상태 요약을 반환한다.

        Returns
        -------
        dict
            Dict containing ``history_len``, ``unique_actions``, ``most_common``,
            ``most_common_freq``, ``warn_issued`` keys.
    ...
    def reset(self) -> None:
        """Clear the history and internal state.
        기록과 내부 상태를 초기화한다."""
...
def _safe_hash(tool_args: dict) -> int:
    """Safely hash an argument dict that may contain nested dicts.
    중첩 dict 를 포함한 인자를 안전하게 해시한다.

    Falls back to ``repr`` when a value cannot be converted to ``frozenset``.
    ``frozenset`` 으로 변환 불가능한 값이 있으면 ``repr`` 로 폴백한다.
    """
    try:
        return hash(frozenset(tool_args.items()))
    except TypeError:
        # repr-based hash when nested dict / list / unhashable values are present.
